...
#define assert_empty(s)                               \
    {                                                 \
        test_assert(stack_int_empty(s), "not empty"); \
    }
#define assert_not_empty(s)                        \
    {                                              \
        test_assert(!stack_int_empty(s), "empty"); \
    }
#define assert_full(s)                              \
    {                                               \
        test_assert(stack_int_full(s), "not full"); \
    }
#define assert_not_full(s)                       \
    {                                            \
        test_assert(!stack_int_full(s), "full"); \
    }
#define assert_pop(s, expected)                                      \
    {                                                                \
        int actual = stack_int_pop(s);                               \
        char str[50];                                                \
        sprintf(str, "popped %d but expected %d", actual, expected); \
